...
Always develop inside a `venv` or `pipenv` to avoid bleeding global packages.

# Pre-commit Hook
Install `pre-commit` and run `pre-commit install` so Black, Ruff, and Mypy fire automatically.

# Commit Messages
<type>(scope): concise summary  (e.g., `feat(auth): add JWT refresh endpoint`)
